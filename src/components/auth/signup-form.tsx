  ...
  return (
    <div className="w-full max-w-md p-8 space-y-8 bg-card rounded-xl shadow-2xl">
      <h1 className="text-3xl font-bold text-center text-foreground">
        {t('signup.title', 'Create your SimulTradex Account')} (Disabled)
      </h1>
      <p className="text-center text-muted-foreground">
        {t('auth.disabled.featureUnavailable', 'Account creation is currently disabled because user authentication has been removed from the application.')}
      </p>
      ...
      <div className="text-sm text-center text-muted-foreground">
        ...
        <span className="font-medium text-primary opacity-50">
          {t('signup.loginLink', 'Log in (Disabled)')}
        </span>
      </div>
    </div>
  );

...
import { BarChartBig, History, Wallet, LayoutDashboard, Languages as LanguagesIcon, ChevronDown, Copyright, Settings, Check } from 'lucide-react'; // Added Check
...
export function MainLayout({ children }: { children: React.ReactNode }) {
  ...
  const footerContent = currentYearValue ? (
    <>
      <div className="mt-4 p-2 text-center text-xs text-muted-foreground group-data-[collapsible=icon]:hidden">
        <p>&copy; {currentYearValue} {hydrated ? t('app.name', DEFAULT_APP_NAME) : DEFAULT_APP_NAME}.</p>
        <p>{hydrated ? t('footer.createdBy', 'Creado con IA por Allan Soto') : 'Creado con IA por Allan Soto'}</p>
      </div>
  ...
  ) : null; // Render null until currentYearValue is available
        ...
        <SidebarHeader className="p-2">
          <Link href="/dashboard" className="flex items-center gap-2 px-2 py-2">
            ...
            <span className="text-lg font-semibold group-data-[collapsible=icon]:hidden">
              {hydrated ? t('app.name', DEFAULT_APP_NAME) : DEFAULT_APP_NAME}
            </span>
          </Link>
        </SidebarHeader>
        ...
        <SidebarContent className="flex-1 overflow-y-auto">
          <SidebarMenu>
            <SidebarMenuItem>
              <SidebarMenuButton
                ...
                tooltip={{content: hydrated ? t('dashboard.title') : 'Dashboard', side: 'right', align: 'center' }}
              >
                <Link href="/dashboard">
                  <LayoutDashboard className="h-5 w-5" />
                  <span className="group-data-[collapsible=icon]:hidden">
                    {hydrated ? t('dashboard.title') : 'Dashboard'}
                  </span>
                </Link>
              </SidebarMenuButton>
            </SidebarMenuItem>
            <SidebarMenuItem>
              <SidebarMenuButton
                ...
                tooltip={{ content: hydrated ? t('settings.accountSettings', 'Account Settings') : 'Account Settings', side: 'right', align: 'center' }}
              >
                <Link href="/account">
                  <Settings className="h-5 w-5" />
                  <span className="group-data-[collapsible=icon]:hidden">
                    {hydrated ? t('settings.accountSettings', 'Account Settings') : 'Account Settings'}
                  </span>
                </Link>
              </SidebarMenuButton>
            </SidebarMenuItem>
            <SidebarMenuItem>
              <SidebarMenuButton
                ...
                tooltip={{content: hydrated ? t('history.menuItem') : 'Order History', side: 'right', align: 'center' }}
              >
                <Link href="/history">
                  <History className="h-5 w-5" />
                  <span className="group-data-[collapsible=icon]:hidden">
                    {hydrated ? t('history.menuItem') : 'Order History'}
                  </span>
                </Link>
              </SidebarMenuButton>
            </SidebarMenuItem>
            <SidebarMenuItem>
              <SidebarMenuButton
                ...
                tooltip={{content: hydrated ? t('balance.menuItem') : 'Balance', side: 'right', align: 'center' }}
              >
                <Link href="/balance">
                  <Wallet className="h-5 w-5" />
                  <span className="group-data-[collapsible=icon]:hidden">
                    {hydrated ? t('balance.menuItem') : 'Balance'}
                  </span>
                </Link>
              </SidebarMenuButton>
            </SidebarMenuItem>
          ...
          <SidebarMenuItem>
            <SidebarMenuButton
              ...
              tooltip={{content: hydrated ? t('settings.language') : 'Language', side: 'right', align: 'center' }}
            >
              <div className="flex items-center">
                <LanguagesIcon className="h-5 w-5" />
                <span className="group-data-[collapsible=icon]:hidden ml-2">
                  {hydrated ? t('settings.language') : 'Language'}
                </span>
              </div>
              <ChevronDown
                className={cn(
